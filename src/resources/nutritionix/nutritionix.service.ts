...
import { NutritionixNutrientDto, NutritionixNutrient } from './dto/nutrient.dto';
import { NutritionixIngredientSearchResult } from './dto/search-ingredient.dto';

export type NutrientMap = Record<number, NutritionixNutrientDto>;
...
export class NutritionixService {
  ...
  getNutrients(): Observable<NutritionixNutrientDto[]> {
    return this.http
      .get<NutritionixNutrient[]>(`${NUTRITIONIX_API_URL}/v2/utils/nutrients`, { headers })
      .pipe(
        map((res) => camelizeKeys(res.data) as NutritionixNutrientDto[]),
        tap((nutrients) => {
